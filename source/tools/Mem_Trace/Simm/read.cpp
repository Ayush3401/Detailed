...
// 		{
// 			if(page_counter[node_no]%2==1 && L.free_local())
// 			{
...
// A:
	...
	if(_pud==nullptr && L.free_local())
	{
		_pgd.add_in_pgd(pgd_vaddr);
		_pud=_pgd.access_in_pgd(pgd_vaddr);
	}
	else if(_pud==nullptr && !L.free_local())
		return -1;

	_pmd=_pud->access_in_pud(pud_vaddr);
	if(_pmd==nullptr && L.free_local())
	{
		_pud->add_in_pud(pud_vaddr);
		_pmd=_pud->access_in_pud(pud_vaddr);
	}
	else if(_pmd==nullptr && !L.free_local())
		return -1;

	_pte=_pmd->access_in_pmd(pmd_vaddr);
	if(_pte==nullptr && L.free_local())
	{
		_pmd->add_in_pmd(pmd_vaddr);
		_pte=_pmd->access_in_pmd(pmd_vaddr);
	}
	else if(_pte==nullptr && !L.free_local())
		return -1;

	_page=_pte->access_in_pte(pte_vaddr);
	if(_page==nullptr && L.free_local())
	{
	...
	else if(_page==nullptr && !L.free_local())
		return -1;
...
void *local_handler(void *node)
{
	...
	{	
		...
		uint64_t current_request_cycle=ceil((double)(temp.cycle)/1.7);
		long double current_window=current_request_cycle / window_cycles;
		
		if(current_window <=window_no && !trace_in.eof())
		{
			//tra.push_back(temp);
			...
			else if(local_paddr==-1)
			{
				...
				remote_access.cycle=current_request_cycle+tx_dis_latency;//one-side dis-latency
