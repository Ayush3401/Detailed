...
void write_back_to_memory(int, int, int, int16_t, uint64_t, CACHE_BASE::ACCESS_TYPE);
...
void update_prefetch_params(int);
void LLC_to_memory(int);
void prefetch(miss_queue, int);
#include "page_migration.cpp"
...
uint64_t mem_trans_id[num_nodes] = {1};
...
vector<vector<uint64_t>> top_strides(num_nodes);
...
void itlb_search(int node_id, int core_id)
{
    ...
    if (!core_inst_stream[node_id][core_id].empty())
    {
        ...
        if (temp.is_Branch)
        {
            ...
            {
                // right branch prediction, do not fetch in this cycle
...
// updating the page bits of the victim and new tlb entry to be used by the TLB Aware clock replacemrnt
#ifdef migration
            update_page_bits(_pgd[node_id], vaddr, paddr, node_id, 0);
            // update_page_bits(_pgd[node_id], victim_vaddr, victim_paddr, node_id, 1);
            // add the reverse mapping of the new TLB entry
            uint64_t map_vaddr = vaddr >> 12;
            add_reverse_map(paddr, map_vaddr);
#endif
            ...
            for (int i = 0; i < line_read; i++)
            {
                if (line_hit[i] == 0)
                {
                    ...
                    if (victim_modified)
                    {
                        ...
                        write_back_to_memory(node_id, core_id, cache_level, (*it).inst_fetch.ins.proc_id, victim_addr, acctype);
                    }
...
void update_l3_mshr(int node_id)
{
    ...
    bool ok = 0;
    int temp_coreid = coreid;
    while (!ok)
    {
        coreid = (coreid + 1) % core_count;
        if (temp_coreid == coreid) // to stop while loop, if same core is given chance again
            ok = 1;
            ...
            for (int i = 0; i < line_read; i++)
            {
                if (line_hit[i] == 0)
                {
                    ...
                    if (victim_modified)
                    {
                        ...
                        // need address of replaced block
                        write_back_to_memory(node_id, (*it).core_id, cache_level, (*it).inst_fetch.ins.proc_id, victim_addr, acctype);
                    }
                    GetNextBlockAddress(paddr, (*it).inst_fetch.ins.proc_id);
                }
            }
...
////////////////////////////////////////// My Code ///////////////////////////////////////////////////////////////
void update_prefetch_params(int node_id)
{
    uint64_t total_sum = 0;
    for (uint64_t a = 0; a < MAX_STRIDE; a++)
    {
        total_sum += strides[node_id][a];
    }
    top_strides[node_id].clear();
    for (uint64_t j = 0; j < MAX_STRIDE; j++)
    {
        if (strides[node_id][j] > total_sum / 8)
        {
            top_strides[node_id].push_back(j + 1);
        }
    }
}
////////////////////////////////////////// My Code ///////////////////////////////////////////////////////////////
...
void LLC_to_memory(int node_id)
{
    for (auto it = l3_miss_buffer[node_id].begin(); it != l3_miss_buffer[node_id].end(); it++)
    {
        if ((*it).L3_cache_miss_complete_cycle <= common_clock && (*it).L3_cache_miss_complete_cycle > 0)
        {
            ...
            if (history[node_id].size() == 100 && !is_training_complete[node_id])
            {
                is_training_complete[node_id] = 1;
                update_prefetch_params(node_id);
            }
            if (history[node_id].size() >= 100 && miss_entry.source_cache == 'D')
                prefetch(miss_entry, node_id);
...
void prefetch(miss_queue miss_entry, int node_id)
{
    for (uint64_t i = 0; i < top_strides[node_id].size(); i++)
    {
        uint64_t vaddr = miss_entry.ins_vaddr, paddr = NULL;
        uint64_t block_addr = vaddr & (0xffffffffffc0);
        block_addr >>= 6;
        block_addr += top_strides[node_id][i];
        vaddr = block_addr << 6;
        bool page_found_in_page_table = false;
        page_found_in_page_table = page_table_walk(_pgd[node_id], vaddr, paddr);
        if (page_found_in_page_table)
        {
            miss_queue prefetch_entry = miss_entry;
            prefetch_entry.mem_trans_id = mem_trans_id[node_id];
            core_map[node_id][mem_trans_id[node_id]] = prefetch_entry.core_id;
            mem_trans_id[node_id]++;
            prefetch_entry.paddr = paddr;
            prefetch_entry.ins_vaddr = vaddr;
            cout << "Prefetch " << miss_entry.ins_vaddr << " " << vaddr << "\n";
            send_to_memory(prefetch_entry, node_id, 0);
        }
    }
}
...
void send_to_memory(miss_queue miss_new_entry, int node_id, bool is_write_back)
{
...
#ifdef migration
        if (!is_write_back)
            update_page_bits(_pgd[node_id], miss_new_entry.ins_vaddr, page_addr, node_id, 0);
        update_migration_benefit(node_id, page_addr); // to update the benefit accesses
    ...
    else
    {
        num_remote[node_id]++;
#ifdef migration
        if (migration_flag[node_id] == 0)
            update_hash_table_access_count(page_addr, node_id); // counting hot pages
#endif
