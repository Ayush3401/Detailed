...
void *remote_pool_handler(void *pool)
{
	...
	while (1)
	{
		...
		while (packet_queue_pool[remote_pool].size() > 0 && packet_queue_pool[remote_pool].front().mem.cycle <= pool_clock)
		{
		...
		cout << "pool " << pool_clock << " " << node_clock << " " << packet_queue_pool[remote_pool].size()
			 << "\n";
		if (remote_pool == 0)
		{
			pool_clock++;
			while (pool_clock > node_clock && rem_flag==false)
			;
		}
