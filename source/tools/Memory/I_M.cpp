...
using namespace std;
#define CORE_NUM 8
#define TLB_PENALTY 7
#define L1_PENALTY 4
#define L2_PENALTY 12
#define L3_PENALTY 41
...
static VOID InsRef_No_INS(UINT32 threadid, ADDRINT addr, char r, UINT32 procid)
{
    BOOL Miss = false;
    BOOL ul2Hit = false;
    BOOL ul3Hit = false;
...
static std::size_t PenaltyCalc(BOOL bool_tlbHit , BOOL bool_l1Hit, BOOL bool_l2Hit, BOOL bool_l3Hit)
{
    ...
    return (int_tlbHit * TLB_PENALTY) + (int_l1Hit * L1_PENALTY) + (int_l2Hit * L2_PENALTY) + (int_l3Hit * L3_PENALTY);
}
  ...
  //  printf("\n*bufno=%ld",*buf_no);
    for(UINT64 i=0;i<*buf_no;i++)
    {
        if(app->buf[i].buf_type==1)
            InsRef_No_INS(app->buf[i].threadid,app->buf[i].addr,app->buf[i].r, app->buf[i].procid);
...
{     
    ...
    ins_stat<<"\nTotal instructions traced in Node-"<<Nodeid<<" App-"<<Procid<<" : "<<ins_count<<endl;
