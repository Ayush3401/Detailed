...
// memory size in GBs (used by memory management unit)
...
vector<unordered_set<uint64_t>> history(num_nodes);
vector<vector<uint64_t>> strides(num_nodes, vector<uint64_t>(10));
int prefetch_distance = 2;
vector<bool> is_training_complete(num_nodes);
const uint64_t MAX_STRIDE = 10;
vector<uint64_t> last_processed_epoch(num_nodes);
...
int total_core_local_time[num_nodes][core_count] = {0};
int total_core_local_count[num_nodes][core_count] = {0};
// used to store average remote memory access cycle time for different nodes
int total_core_remote_time[num_nodes][core_count] = {0};
int total_core_remote_count[num_nodes][core_count] = {0};
// used to store average remote memory access for different pools
int total_remote_pool_time[num_mem_pools] = {0};
int total_remote_pool_count[num_mem_pools] = {0};
...
void update(int node_id, int mem_id, uint64_t cycle, uint64_t tid, uint64_t start_cycle, uint64_t miss_cycle, uint64_t address)
{
	//	uint64_t size=0;
	int i = mem_id - 1;
	total_cycle[i] = cycle;
...
void some_object::read_complete(int nid, uint64_t tid, uint64_t start_cycle, uint64_t miss_cycle, int mem_id, unsigned id, uint64_t address, uint64_t clock_cycle)
{
	...
	update(nid, mem_id, clock_cycle, tid, start_cycle, miss_cycle, address);
...
void add_local_access_time(remote_memory_access mem_response)
{
	...
	int trans_id = mem_response.trans_id;
