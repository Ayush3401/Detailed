...
exe_units execution_unit[num_nodes][core_count][num_exec_units];

int has_exe_unit(int node_id, int core_id)
{
        int index = 0;
        for (index = 0; index < num_exec_units; index++)
        {
                if (execution_unit[node_id][core_id][index].status == 0)
                {
                        return index;
                }
        }
...
deque<pair<int, int>> ready_queue[num_nodes][core_count];
...
void decode_ins(int node_id, int core_id)
{
        ...
        {
                // if(common_clock>30900000)cout<<"decode"<<endl;
...
{
        int count = 0;
        while (count < dispatch_width && !ready_queue[node_id][core_id].empty())
        {
                // if(common_clock>30900000)cout<<"dispatch"<<endl;
                int rs_id = ready_queue[node_id][core_id].front().first;
                int cycle = ready_queue[node_id][core_id].front().second;
                int exec_index = has_exe_unit(node_id, core_id);
...
void update_exec_units(int node_id, int core_id)
{
        ...
        {
                if (execution_unit[node_id][core_id][i].status == 1)
                        execution_unit[node_id][core_id][i].cycle_completed++;
        }
}
// To store the completed instructions rob entry to broadcast
vector<int> completed_inst_rob_index[num_nodes][core_count];

// check if any inst has been completed i.e present forr 10 cycles remove from exec unit
// and add it in  completed_inst_rob_index
void check_executed_instruction(int node_id, int core_id)
{
        for (int i = 0; i < num_exec_units; i++)
        {
                if (execution_unit[node_id][core_id][i].cycle_completed == 10 && execution_unit[node_id][core_id][i].status)
                {
                        num_inst_exec[node_id][core_id]++;
                        execution_unit[node_id][core_id][i].status = 0;
                        int rob_index = execution_unit[node_id][core_id][i].rob_index;
        ...
        for (int i = 0; i < completed_inst_rob_index[node_id][core_id].size(); i++)
        {
                ...
                while (reorder_buffer[node_id][core_id].rob[rob_index].INS_RS_Dependency.size())
                {
                        ...
                        if (cleared)
                        {
                                if (rs_index == -1)
                                {
                                        cout << "rs_index is -1 (broadcast_ins_com)";
                                        cin.get();
                                }
                                abc.open("ready.txt", ios::app);
                                abc << "\n rs_ind: " << rs_index << " rob_ind: " << reservation_station[node_id][core_id][rs_index].ROB_index
                                    << " ins_id: " << reservation_station[node_id][core_id][rs_index].ins_id;
...
int add_to_load_store_queue(int node_id, int core_id, uint64_t mem_req_addr, int16_t mem_req_size, int proc_id, int rs_index, int rob_index)
{
        ...
        if (load_store_queue[node_id][core_id].size() < LSQ_Size && num_ld_str_inCycle[node_id][core_id] <= ld_str_width)
        {
                ...
                {
                        uint64_t block_addr = (mem_req_addr & (0xffffffffffc0)) >> 6;
                        for (uint64_t a = 0; a < MAX_STRIDE; a++)
                        {
                                uint64_t stride = a + 1;
                                if (history[node_id].find(block_addr - stride) != history[node_id].end())
                                {
                                        strides[node_id][a]++;
                                }
                        }
                        history[node_id].insert(block_addr);
                }
                // UPDATED
                // UPDATED
                // UPDATED
...
void update_load_store_queue(int node_id, int core_id)
{
        while (!pending_load_store[node_id][core_id].empty() && num_ld_str_inCycle[node_id][core_id] <= ld_str_width && load_store_queue[node_id][core_id].size() < LSQ_Size)
        {
                ...
                if (history[node_id].size() < 100)
                {
                        uint64_t mem_req_addr = cache_access.ins_vaddr;
                        uint64_t block_addr = (mem_req_addr & (0xffffffffffc0)) >> 6;
                        // cout << history[node_id].size() << "\n";
                        for (uint64_t a = 0; a < MAX_STRIDE; a++)
                        {
                                uint64_t stride = a + 1;
                                if (history[node_id].find(block_addr - stride) != history[node_id].end())
...
{
        for (int i = 0; i < ready_queue[node_id][core_id].size(); i++)
        {
                ready_queue[node_id][core_id].front().second = ready_queue[node_id][core_id].front().second + 1;
        }
}
...
void issue_instruction(int node_id, int core_id)
{
        ...
        {
                // if(common_clock>30900000)cout<<"issue"<<endl;
...
void update_core(int node_id, int core_id)
{
        ...
        {
                if (common_clock == resume_stall_clock[node_id])
                {
                        migrate_stall[node_id] = 0;
                        migration_flag[node_id] = 0;
