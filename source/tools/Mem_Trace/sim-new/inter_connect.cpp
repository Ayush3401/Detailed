...
// has buffer size of 132MB, (in our experimentation we have 70-ports for 64nodes/6memorypools, or 46-ports with 40nodes/6memorypools)
...
#define tx_input_port_queue_size ((tx_per_port_mb * 1024 * 1024) / 64)	// 64-byte packet while sending
#define rx_input_port_queue_size ((rx_per_port_mb * 1024 * 1024) / 128) // 128-byte packet while receiving
#define tx_output_port_queue_size ((tx_per_port_mb * 1024 * 1024) / 64)
#define rx_output_port_queue_size ((rx_per_port_mb * 1024 * 1024) / 128)

#define nic_bandwidth 100	 // Gbps
#define switch_bandwidth 100 // Gbps
...
bool virtual_queue_has_space(int port_no, int dest_count, deque<packet> (&input_port_queue)[rows][cols], int input_port_queue_size)
{
	int size = 0;
...
void source_to_switch_input_port(deque<packet> *nic_queue_source, deque<packet> (&input_port_queue)[rows][cols], int source_count, int dest_count, int input_port_queue_size)
{
	for (int i = 0; i < source_count; i++)
	{
		...
		if (!virtual_queue_has_space(i, dest_count, input_port_queue, input_port_queue_size))
		{
			for (int j = 0; j < nic_queue_source[i].size(); j++)
			{
				if (nic_queue_source[i].at(j).mem.cycle == node_clock)
...
{
	nic_trans_delay = (tx_packet_size / nic_bandwidth);
	switch_trans_delay = (tx_packet_size / switch_bandwidth);
